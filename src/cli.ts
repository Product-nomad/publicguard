...
async function cmdSchedule(args: string[]): Promise<void> {
  ...
  const onCalendar = validIntervals[interval];
  if (!onCalendar) {
  ...
  mkdirS(unitDir, { recursive: true });

  const servicePath = resolve(unitDir, "publicguard.service");
  const timerPath   = resolve(unitDir, "publicguard.timer");

  const serviceUnit = `[Unit]
Description=PublicGuard secret-leak notifier
...
ExecStart=${nodeExe} ${cliPath} run
StandardOutput=journal
StandardError=journal
`;

  const timerUnit = `[Unit]
Description=Run PublicGuard ${interval}
...
`;
  ...
  writeFileSync(timerPath, timerUnit);
  console.log(`Wrote ${servicePath}`);
  console.log(`Wrote ${timerPath}`);

  execSync("systemctl --user daemon-reload");
  execSync("systemctl --user enable --now publicguard.timer");

  console.log(`\nTimer enabled (${interval}: OnCalendar=${onCalendar})`);
  console.log("Logs: journalctl --user -u publicguard.service");
  console.log("Next run: systemctl --user list-timers publicguard.timer");
}
